...
目前包含 2025 届嘉定、黄浦、徐汇三份一模，共 21 道互动题。每份试卷分别保留题目来源、编号、答案和数学模型；嘉定的两个旧文件链接仍可打开。

| 试卷 | 互动题号 | 进入课件 |
|---|---|---|
| 嘉定 | 10、11、12、16、17、18、20、21 | [8 题](papers/jiading-2025/index.html) |
| 黄浦 | 10、14、16、17、20、21 | [6 题](papers/huangpu-2025/index.html) |
| 徐汇 | 9、11、12、16、18、20、21 | [7 题](papers/xuhui-2025/index.html) |

新试卷沿用粗线、粗体图注、目标红色、深浅主题、完整答案和“按条件画图”。黄浦侧重复平面距离、空间角、递减函数迭代、焦半径条件和极值偏移；徐汇侧重导数变号、圆台展开、函数复合、轮换均值、线面投影及两道新定义/圆锥曲线题。选题理由与数学边界见各试卷目录内的教学说明。

三份课件在电脑上默认**左文右图**。点击工具栏的**“上下排版”**可切为上下阅读，按钮随即变为**“左右排版”**，可一键回到左文右图；浏览器会记住选择。图形与操作控件放在图形栏，题干、看图思路、答案、命题或解析及画图步骤说明放在文字栏，便于边看图边讲题。展开解析时图形宽度保持稳定；切换排版保留参数、三维视角及画图步骤。窄屏自动上下排列，回到宽屏后恢复所选排版。
...
默认使用深色画布，右上角“深色 / 浅色”按钮可切换，并在本浏览器中记住选择。按教室光线与屏幕效果选择合适模式；切换不会改变当前参数、构型或已展开的推导。导航同时显示题号和主题，解析按带小标题的步骤分段。
