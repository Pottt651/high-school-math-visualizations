Problems[18] = {
  ...
  mount(host) {
    ...
    function render(){
      ...
      [Math.PI/4,Math.PI/3].forEach((x,i)=>{p.line([x,-2.6],[x,2.6],{stroke:C.gray,dash:'5 5',width:1});const z=p.to([x,f(x)]);p.add(`<circle cx="${z[0]}" cy="${z[1]}" r="6" fill="white" stroke="${C.ink}" stroke-width="2"/>`);p.math([x,-2.75],i?String.raw`\dfrac\pi3`:String.raw`\dfrac\pi4`,{anchor:'middle',size:18});});
