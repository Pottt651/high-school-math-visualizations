...
公共外壳在模块 `mount(host)` 后，将文字归入 `host` 内的 `.lesson-text`；控件 `.controls` 和图形 `.figures` 保留在 `host` 内的右侧区域。桌面默认左文右图（登记表 `defaultLayout: "side"`），按钮显示下一种排列“上下布局”或“左右布局”，不提供左图右文。`body.side-layout` 表示当前启用桌面左右布局；窄屏自动上下排列，保留宽屏偏好。左右模式的两栏在隐藏关键关系时仍存在，展开解析不得改变图形宽度。题干、看图思路、答案、命题或解析及构图说明留在文字栏，控件与图形留在图形栏。新增题目沿用这项约定，不按某个题号添加跨试卷生效的布局特例。

归类后所有模块节点仍在原 `host` 中，题目查询和事件委托继续限定在 `host` 内；不要假定 `.readout` 等文字节点必须是 `host` 的直接子元素。使用明确的图形 ID，不能用首个 `svg` 查询代替绘图画布（公式内部也有 SVG）。排版切换仅重排已有节点与显示状态，不重新执行 `mount`、`reset` 或重建数学模型，保留参数、相机、解析显示与构图步骤。偏好写入浏览器本地存储键 `math-visualizations-display`，兼容读取另一设备版本的 `math-visualizations-layout`；不可用时不影响本次切换。左右布局中的文字栏支持键盘聚焦和续读提示；分步绘图仍须保持左右布局，不能被旧的纵向 flex 规则覆盖。
