...
window.M = (() => {
  ...
  function render(tex,display=false,bold=false){
    const key=String(display)+':'+bold+':'+tex;
    if(!htmlCache.has(key)){
      if(htmlCache.size>1800)htmlCache.clear();
      let markup=katex.renderToString(bold?String.raw`\boldsymbol{${tex}}`:String(tex),{displayMode:display,throwOnError:true,strict:'ignore',trust:false,output:'htmlAndMathml'});
      // Weight is presentation; retain the original mathematical source annotation.
      if(bold)markup=markup.replace(/(<annotation encoding="application\/x-tex">)[\s\S]*?(<\/annotation>)/,(_,open,close)=>open+esc(tex)+close);
      htmlCache.set(key,markup);
    }
