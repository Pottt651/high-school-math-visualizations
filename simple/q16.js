Problems[16] = {
  ...
  mount(host) {
    host.innerHTML = `
      ...
      <div class="explain">
        <p><strong>① 假。</strong>${M.inline('r=2')} 时，“只有有限个初值使数列严格递增”不成立。任取 ${M.inline(String.raw`a_1\in\left(0,\frac12\right)`)}，区间 ${M.inline(String.raw`\left(0,\frac12\right)`)} 在递推中保持，且 <span class="target">${M.inline(String.raw`a_{n+1}-a_n=a_n(1-2a_n)>0`)}</span>。</p>
        <p><strong>② 真。</strong>命题：${M.inline('r=2')} 时，存在初值及正整数 ${M.inline('P')}，使 ${M.inline('n>P')} 时 <span class="target">${M.inline(String.raw`a_{n+1}-a_n<\frac1{2025}`)}</span>。取 ${M.inline(String.raw`a_1=\frac12`)}，则任意 ${M.inline('n')} 的差都是 ${M.inline('0')}，任取正整数 ${M.inline('P')} 即可。<strong>原题差值没有绝对值。</strong></p>
        <p><strong>③ 真。</strong>命题：${M.inline('r=3')} 时，存在初值及正整数 ${M.inline('P')}，使 ${M.inline('n>P')} 时 <span class="target">${M.inline('a_{n+1}=a_n')}</span>。取 ${M.inline(String.raw`a_1=\frac23`)}，所有项精确等于 ${M.inline(String.raw`\frac23`)}；取 ${M.inline(String.raw`a_1=\frac13`)}，也在一步后到达 ${M.inline(String.raw`\frac23`)}。这两种初值都满足命题。</p>
        <p><strong>④ 假。</strong>${M.inline('r=-3')} 时，“不存在使 ${M.inline(String.raw`n\ge3`)} 的项全为正的初值”不成立：<span class="target">${M.inline(String.raw`a_1=\frac12\ \Rightarrow\ a_2=-\frac34\ \Rightarrow\ a_3=\frac{63}{16}>\frac43`)}</span>；${M.inline(String.raw`x\ge\frac43`)} 时，${M.inline(String.raw`3x(x-1)\ge x`)}，故后续各项始终为正。</p>
        <p><strong>“接近”为什么不等于“最终相等”？</strong>${M.inline('r=2')} 时令 ${M.inline('b_n=1-2a_n')}，则 ${M.inline('b_{n+1}=b_n^2')}。若 ${M.inline(String.raw`a_1\ne\frac12`)}，则 ${M.inline(String.raw`b_1\ne0`)}，每个有限项的 ${M.inline('b_n')} 都不为 ${M.inline('0')}。电脑显示相等可能仅仅是舍入。</p>
      </div>`;
