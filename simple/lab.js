...
window.Lab = (() => {
  ...
  function plot(svg,b){
    ...
    function screenText(x,y,t,o={}){chunks.push(`<text x="${x+(o.dx||0)}" y="${y+(o.dy||0)}" font-size="${o.size||18}" fill="${escape(o.color||C.ink)}" text-anchor="${o.anchor||'start'}"${o.weight?` font-weight="${o.weight}"`:''} paint-order="stroke" stroke="var(--canvas)" stroke-width="3" stroke-linejoin="round">${escape(t)}</text>`);}
    ...
    const api={w,h,sx,sy,to,
      ...
      dot:(p,label='',color=C.blue,dx=10,dy=-10)=>{if(!finite(p))return;const q=to(p);chunks.push(`<circle cx="${q[0]}" cy="${q[1]}" r="5" fill="${escape(color)}" stroke="var(--canvas)" stroke-width="2"/>`);if(label)screenText(q[0]+dx,q[1]+dy,label,{color,size:18});},
