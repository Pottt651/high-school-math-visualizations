Problems[21] = {
  ...
  mount(host){
    const C=Lab.C;let kind='sin',x0=0,a=-.5,atOpt=false,sinK=null,sinShift=0,viewMax=6,leftPlot,drag=false,notice='';
    ...
    function render(){
      ...
      const formula=kind==='sin'?String.raw`\sin x`:kind==='line'?'-x':kind==='quad'?'x^2+x+1':String.raw`\ln x+ax^2`;
      const p=Lab.plot(left,{xmin,xmax,ymin,ymax,equal:false,pad:46});leftPlot=p;p.axes();p.curve(fn,xmin,xmax,{stroke:C.blue,width:2.8,n:500});p.line([xmin,fv],[xmax,fv],{stroke:C.target,dash:'6 4',width:2.3});
      if(x0>=xmin&&x0<=xmax){p.dot([x0,fv],'',C.target);p.math([x0,fv],'x_0',{color:C.target,dx:8,dy:-12,size:18});p.line([x0,0],[x0,fv],{stroke:C.target,dash:'4 4',width:1});}
      if(witness!==null&&witness>=xmin&&witness<=xmax)p.dot([witness,fn(witness)],'反例点',C.gold,8,-14);
      p.screenMath(16,27,`f(x)=${formula}`,{size:19});p.screenMath(16,p.h-18,String.raw`\text{候选值 }f(x_0)=${Lab.fmt(fv,4)}`,{size:18,color:C.target});p.finish();
      ...
      q.poly([[xmin,low],[xmax,low],[xmax,high],[xmin,high]],{fill:C.green,opacity:.11,stroke:'none'});q.axes();q.curve(x=>fn(x)-fv,xmin,xmax,{stroke:C.blue,width:2.8,n:500});q.line([xmin,0],[xmax,0],{stroke:C.green,width:1.8});if(kind==='sin')q.line([xmin,-2],[xmax,-2],{stroke:C.green,width:1.5,dash:'4 4'});
      if(x0>=xmin&&x0<=xmax){q.dot([x0,0],'',C.target);q.math([x0,0],'x_0',{color:C.target,dx:8,dy:-12,size:18});}if(witness!==null&&witness>=xmin&&witness<=xmax)q.dot([witness,diff],'不在 A 中',C.gold,8,diff>0?-15:24);
      q.screenMath(16,27,String.raw`\text{差值 }f(x)-f(x_0)`,{size:19});q.screenMath(16,q.h-18,kind==='sin'?'A=[-2,0]':isMin?String.raw`A=[0,+\infty)`:String.raw`A=(-\infty,0]`,{size:18,color:C.green});q.finish();
      ...
      host.querySelector('#q21-explain').innerHTML=kind==='sin'?`<p>${M.inline(String.raw`\sin x`)} 的值域为 ${M.inline('[-1,1]')}。整体减去 ${M.inline('1')} 后恰为 ${M.inline('[-2,0]')}，故 ${M.inline('A')} 值为 ${M.inline('1')}，取值位置为</p>${M.block(String.raw`x_0=\frac\pi2+2k\pi,\quad k\in\mathbb Z`)}`:kind==='line'?`<p>${M.inline(String.raw`A=[0,+\infty)`)} 要求 ${M.inline(String.raw`f(x)\ge f(x_0)`)}，即取到全局最小值。</p><p>对 ${M.inline('-x')}，任给 ${M.inline('x_0')}，令 ${M.inline('x=x_0+1')}，差值恒为 ${M.inline('-1')}，所以不存在 ${M.inline('A')} 值。</p>`:kind==='quad'?`<p>${M.inline(String.raw`A=[0,+\infty)`)} 要求全局最小值。</p>${M.block(String.raw`x^2+x+1=\left(x+\frac12\right)^2+\frac34`)}<p>取 ${M.inline(String.raw`x_0=-\frac12`)} 后，差值变为 ${M.inline(String.raw`\left(x+\frac12\right)^2\ge0`)}，所以 ${M.inline('A')} 值为 ${M.inline(String.raw`\frac34`)}。</p>`:`<p>${M.inline(String.raw`A=(-\infty,0]`)} 要求全局最大值。</p><p>${M.inline(String.raw`a\ge0`)} 时取 ${M.inline('x=2x_0')}，差值为 ${M.inline(String.raw`\ln2+3ax_0^2>0`)}，任一候选点都可被否决。</p><p>${M.inline('a<0')} 时，</p>${M.block(String.raw`f'(x)=\frac1x+2ax`)}<p>在 ${M.inline(String.raw`x^*=\sqrt{-\frac1{2a}}`)} 左侧为正、右侧为负，唯一最大值为</p>${M.block(String.raw`-\frac{\ln(-2a)+1}{2}`)}<p>${M.inline(String.raw`a\to0^-`)} 时极值点趋于 ${M.inline(String.raw`+\infty`)}，不等于不存在最大值。</p>`;
    }
    selector.onchange=()=>{kind=selector.value;atOpt=false;sinK=null;sinShift=0;notice='';viewMax=6;x0=kind==='log'?1.6:0;render();};range.oninput=()=>setX(+range.value);number.onchange=()=>setX(number.value.trim()===''?NaN:+number.value);aInput.onchange=()=>{
      ...
      a=v;notice='';if(atOpt&&a<0){x0=optimum();viewMax=Math.max(6,x0*1.8);}else atOpt=false;render();
    };
    ...
    render();return{render,reset(){kind='sin';x0=0;a=-.5;atOpt=false;sinK=null;sinShift=0;notice='';viewMax=6;selector.value=kind;render();},getState(){return{kind,x0,a,atOpt,sinK,analyticPosition:kind==='sin'&&atOpt?{formula:'π/2+2kπ',k:sinK}:null,viewMax,optimalX:optimum(),f0:atOpt?extValue():fn(x0),notice};}};
  }
};
