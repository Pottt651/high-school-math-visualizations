...
(async()=>{const browser=await launchBrowser(),page=await browser.newPage(),errors=[],network=[],shots=path.join(__dirname,'screenshots/2024-js');fs.mkdirSync(shots,{recursive:true});
...
await page.goto(pathToFileURL(path.resolve(__dirname,'../papers/jinshan-2024/index.html')).href);
...
for(const [width,height] of [[1280,720],[1920,1080]]){await page.setViewportSize({width,height});for(const id of [9,11,17,18,19,20,21]){
...
await page.evaluate(()=>Lesson.select(11));await page.locator('#js24-11-seven').click();assert.equal((await page.evaluate(()=>Lesson.getState())).roots.length,7);
await page.evaluate(()=>Lesson.select(18));await page.locator('#js24-18-boundary').click();assert.equal((await page.evaluate(()=>Lesson.getState())).firstDifference,0);
await page.evaluate(()=>Lesson.select(21));await page.locator('#js24-21-part').selectOption('3');await page.locator('#js24-21-first').click();assert.equal((await page.evaluate(()=>Lesson.getState())).n,15);
await page.setViewportSize({width:390,height:844});for(const id of [9,11,17,18,19,20,21]){await page.evaluate(id=>Lesson.select(id),id);await settle();assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth)<=392);}
assert.deepEqual(errors,[]);assert.deepEqual(network,[]);await browser.close();console.log('PASS JS2024 all seven modules, modes, preset controls, proof widths, theme state, desktop/mobile offline.');})().catch(reportFailure);
