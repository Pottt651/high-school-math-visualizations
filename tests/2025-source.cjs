...
const root=path.resolve(__dirname,'..'),papers=JSON.parse(fs.readFileSync(path.join(root,'papers/catalog.json'),'utf8')).filter(p=>p.year===2025),issues=[];
...
assert.deepEqual(issues,[],'Use String.raw for TeX commands in ordinary literals');console.log('PASS: registered 2025 source syntax, TeX string escaping and control-character checks.');
