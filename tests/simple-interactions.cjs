...
(async()=>{
  ...
  await drag(page.locator('#q16-web circle[fill="var(--plot-target)"]').first(),35,0);assert.notEqual((await state()).initial,.25);
  await select(20);const theta=(await state()).theta;
  await drag(page.locator('#q20-ellipse circle[fill="var(--plot-blue)"]').first(),30,45);
  ...
  await drag(page.locator('#q20-ellipse circle[fill="var(--plot-gold)"]').first(),32,21);
