...
const year=catalog.filter(p=>p.year===2025);assert.equal(year.length,7);assert.equal(year.reduce((n,p)=>n+p.questions.length,0),49);for(const[id,ids]of Object.entries({'jiading-2025':[10,11,12,16,17,18,20,21],'huangpu-2025':[10,14,16,17,20,21],'xuhui-2025':[9,11,12,16,18,20,21]}))assert.deepEqual(year.find(p=>p.id===id).questions.map(q=>q.id),ids,'Existing 2025 selections must be preserved');
const papers=catalog.filter(p=>p.year===2025&&['jingan-2025','putuo-2025','yangpu-2025','jinshan-2025'].includes(p.id)&&(!process.env.PAPER_2025||p.id===process.env.PAPER_2025));
(async()=>{
 assert.ok(papers.length);if(!process.env.PAPER_2025||process.env.REQUIRE_ALL_2025){assert.equal(papers.length,4);assert.equal(papers.reduce((n,p)=>n+p.questions.length,0),28);}
 const browser=await launchBrowser(),errors=[],external=[],issues=[],shots=path.join(__dirname,'screenshots/2025');fs.mkdirSync(shots,{recursive:true});
 for(const paper of papers){
  ...
  const temp=fs.mkdtempSync(path.join(os.tmpdir(),'math2025-portable-')),filename=paper.build.legacyOutputs[0],destination=path.join(temp,filename);fs.copyFileSync(path.join(root,filename),destination);
 ...
 fs.writeFileSync(path.join(shots,'classroom-report.json'),JSON.stringify({errors,external,issues},null,2));await browser.close();assert.deepEqual(errors,[]);assert.deepEqual(external,[]);assert.deepEqual(issues,[]);console.log('PASS: 2025 classroom controls, math rendering, layout preservation, 720p/1080p, and isolated offline single-file use.');
})().catch(reportFailure);
